 ...
 * @FilePath: /vue3-element-admin/src/views/system/dept/index.vue
...
<template>
  <div class="dept-page w-full bg-white p-2.5">
    ...
    <el-dialog
      :title="i18nSystemDept(dialogTitle)"
      :modelValue="dialogOpen"
      width="30%"
      :before-close="onBeforeClose"
    >
      <div class="mb-2">
        根节点为 0, 二级节点为 1 或者 2,等, 三级几点为 10 或者 20, 21,等,
        更多节点 以此类推
      </div>
      <el-form ref="form" :model="deptForm" label-width="100px">
        ...
        <el-form-item :label="i18nGlobal('id')">
          <el-input
            v-model="deptForm.id"
            :placeholder="i18nGlobal('idPlaceholder')"
          ></el-input>
        </el-form-item>
        <el-form-item :label="i18nSystemDept('dialog.deptName')">
          <el-input
            v-model="deptForm.name"
            :placeholder="i18nSystemDept('dialog.deptNamePlaceholder')"
          ></el-input>
        </el-form-item>
        <el-form-item :label="i18nSystemDept('dialog.deptDesc')">
          <el-input
            v-model="deptForm.description"
            :placeholder="i18nSystemDept('dialog.deptDescPlaceholder')"
          ></el-input>
        </el-form-item>
...
const parentName = computed(() => {
  return deptForm.value.parentId === 0
    ? "根节点"
    : isOpenSetParentName(deptForm.value.parentId);
});
